...
Antes de ajustar cualquier modelo, es fundamental entender la estructura de los datos, detectar posibles problemas (como valores faltantes o outliers), y elegir adecuadamente las variables que se utilizarán como predictores del `happiness_score`.
...
Como se puede observar, las correlaciones más fuertes con happiness_score corresponden a:
...
Por otro lado, también puede emplearse una estrategia bottom-up, en la que se comienza con un modelo nulo y se añaden variables una a una en función de su mejora sobre el ajuste del modelo.
...
Adicionalmente, se ha probado una estrategia bottom-up (hacia adelante), comenzando desde un modelo reducido, e incorporando variables una a una. El AIC mejora progresivamente, alcanzando finalmente el modelo óptimo añadiendo todas las variables; un modelo diferente al antes mencionado.
...
El p-valor obtenido es extremadamente bajo (p-value = 2.195e-10), lo que indica que los residuos no siguen una distribución normal. Esto también se confirma visualmente en el gráfico Q-Q, donde los residuos llega un momento en el que se desvían de la línea teórica.
...
Si observamos la gráfica Residuals vs Fitted, podríamos decir que los residuos están uniformemente dispersos alrededor del eje de abscisas, por lo que se cumple la condición de que los residuos tienen media cero.
...
El gráfico de residuos frente a los valores ajustados también muestra un patrón creciente, indicio visual de heterocedasticidad. El test devuelve un p-valor < 2.2e-16, por lo que rechazamos la hipótesis nula de homocedasticidad. Esto indica que hay heterocedasticidad en los residuos, es decir, su varianza no es constante.
...
$$
\text{happiness}_{ij} = \beta_0 + \beta_1 \cdot \text{support}_{ij} + \beta_2 \cdot \text{lifeexp}_{ij} + \beta_3 \cdot \text{freedom}_{ij} + \beta_4 \cdot \text{gdp}_{ij} + \beta_5 \cdot \text{corruption}_{ij} + \beta_6 \cdot \text{generosity}_{ij} + \beta_7 \cdot \text{year}_{ij} + u_{0j} + \varepsilon_{ij}
$$
...
Antes de proceder al diagnóstico exhaustivo del modelo propuesto, es importante evaluar si esta especificación es realmente la más adecuada para los datos. Para ello, se comparan múltiples modelos candidatos que varían en los predictores incluidos, y se utiliza un criterio de calidad estadística para seleccionar el mejor. Una de las herramientas más utilizadas para este fin es el criterio de información de Akaike (AIC), que penaliza la complejidad del modelo: un AIC más bajo indica mejor equilibrio entre ajuste y parsimonia. Partiendo del modelo mixto completo previamente planteado, utilizamos el procedimiento que realice una selección automática de predictores que minimice el AIC. Aunque el paquete lmer() utiliza por defecto el método de estimación REML, este método se recomienda cuando el objetivo es estimar los parámetros del modelo final; y no se puede usar para comparar modelos con distintos efectos fijos, ya que el REML estima la varianza teniendo en cuenta las variables fijas incluidas. Por ello, utilizaremos ML, ya que se utiliza cuando quieres comparar modelos que tienen diferentes efectos fijos (es decir, diferentes predictores), y permite comparar modelos usando AIC, BIC o likelihood ratio tests.
...
Si nos fijamos en España, obtiene un Happiness Score previsto de 6.35, situándose en la posición 34 de 160. Esta puntuación refleja un nivel de felicidad moderadamente alto, por encima de la media global. Aunque no alcanza los niveles nórdicos, se encuentra en el tercio superior del ranking, lo que implica una posición destacada entre los países con mayor bienestar percibido. Aunque la puntuación de felicidad predicha para España en 2025 es ligeramente menor (6.35 vs. 6.42), la posición en el ranking mejora: sube del puesto 36 al 34. Esto implica que otros países han bajado más que España, permitiendo su ascenso en el ranking a pesar de una leve caída en su puntuación. Es un buen ejemplo de cómo el ranking depende no solo del valor absoluto, sino también del contexto comparativo entre países.
...
- freedom (β = 1.1587): este coeficiente indica que, controlando por el país y el año, un aumento de una unidad en la percepción de libertad está asociado a un incremento de 1.16 puntos en el índice de felicidad. Es el predictor con mayor impacto del modelo y su t-valor elevado (t = 12.05) confirma su fuerte significancia estadística.

- life_exp (β = 0.0208): aunque el efecto es más modesto en magnitud, resulta altamente significativo (t = 6.11), lo que sugiere que un aumento en la esperanza de vida también contribuye de forma positiva al bienestar subjetivo. Concretamente, una mejora de un punto en esta variable se asocia con un aumento de aproximadamente 0.021 puntos en la felicidad media.

- support (β = 1.2237): este es otro de los predictores clave del modelo. El valor estimado sugiere que, manteniendo las demás variables constantes, un incremento en el nivel percibido de apoyo social se traduce en una subida de 1.22 puntos en el índice de felicidad, lo que representa un efecto considerable. Su elevado t-valor (t = 12.34) respalda la solidez de esta relación.

En conjunto, estos resultados subrayan el papel central de las percepciones sociales y de las condiciones de vida básicas (libertad, apoyo, salud) en la explicación del bienestar subjetivo, incluso una vez consideradas las diferencias estructurales entre países y las variaciones temporales entre años.
