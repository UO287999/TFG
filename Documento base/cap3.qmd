...
Una buena forma de distinguir entre efectos fijos y aleatorios es a partir de ejemplos prácticos en los que podamos apreciar claramente sus diferencias.

En este primer ejemplo, trabajaremos con un modelo que cuenta únicamente con efectos fijos: Regresión Lineal Simple. Este modelo asume que todas las observaciones son independientes y no considera la estructura jerárquica de los datos longitudinales. En este caso, utilizamos un dataset de crecimiento de niños (Orthodont del paquete nlme), donde queremos predecir la distancia entre los dientes (distance) en función de la edad (age).
...
Un segundo ejemplo que puede ayudarnos a entender mejor la diferencia entre efectos es un modelo con efectos aleatorios (Modelo Lineal Mixto). Para corregir la falta de independencia en los datos que hemos visto antes, agregamos un efecto aleatorio para capturar la variabilidad individual de cada sujeto (Subject).
```{r include=FALSE, warning=FALSE}
# Ajustar un modelo mixto con efectos aleatorios por individuo
modelo_mixto <- lmer(distance ~ age + (1 | Subject), data = Orthodont)
# Resumen del modelo
summary(modelo_mixto)
# Extraer la varianza del intercepto por sujeto
var_intercept <- as.numeric(VarCorr(modelo_mixto)$Subject[1])
# Extraer la varianza residual
var_residual <- attr(VarCorr(modelo_mixto), "sc")^2
...
No obstante, podemos extender el modelo incluyendo efectos fijos y aleatorios en la pendiente de age, es decir, permitiendo que la relación entre distance y age varíe entre sujetos.
```{r include=FALSE, warning=FALSE}
# Modelo con efectos fijos y aleatorios en la pendiente
modelo_mixto_completo <- lmer(distance ~ age + (1 + age | Subject), data = Orthodont)
# Resumen del modelo
summary(modelo_mixto_completo)
# Extraer la varianza del intercepto por sujeto
var_intercept_completo <- as.numeric(VarCorr(modelo_mixto_completo)$Subject[1])
var_pendiente_completo <- as.numeric(VarCorr(modelo_mixto_completo)$Subject["age", "age"])
# Extraer la varianza residual
var_residual_completo <- attr(VarCorr(modelo_mixto_completo), "sc")^2
```
...
Si comparamos modelos, el modelo que sólo utiliza efectos fijos asume que todos los individuos siguen la misma relación, además de que ignora la dependencia entre mediciones del mismo sujeto. Por otro lado, el modelo que sólo cuenta con efectos aleatorios permite que cada sujeto tenga su propia intersección, pero, aunque sea mejor que el modelo anterior, sigue sin ser del todo flexible. Para ello, podemos definir un modelo que cuente con efectos tanto fijos como aleatorios, el cual no sólo permite que cada usuario tenga su propia intersección, sino que también tiene su propia tasa de cambio; siendo un modelo mucho más completo, aunque algo más complejo de ajustar.
