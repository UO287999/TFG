...
variables_categoricas <- c("regime_category", "is_democracy",
...
paises_cambio <- c("Angola", "Bolivia", "Burkina Faso", "Burundi", "China", "Comoros",
                   "Congo (Kinshasa)", "Egypt", "Honduras", "Hungary", "Iraq", "Kenya",
                   "Laos", "Liberia", "Madagascar", "Myanmar", "Nicaragua", "Turkey",
                   "Venezuela", "Yemen", "Zimbabwe")

# Variables categóricas de interés
vars_politicas <- c("regime_category", "is_democracy", "electoral_category",
                    "has_free_and_fair_election", "has_alternation", "is_presidential")
...
```{r mapamundi, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
library(plotly)
library(dplyr)
library(sf)
library(rnaturalearth)
...
# Normalizar nombre de países si es necesario
df_sin_democracia_sin_na <- df_sin_democracia_sin_na %>%
  mutate(country = case_when(
...
world_map <- rnaturalearth::ne_countries(scale = "medium", returnclass = "sf") %>%
  left_join(filter(df_sin_democracia_sin_na, year == 2020), by = c("name" = "country"))

# Simplificar geometría (más rápido para plotly)
world_map_s <- st_simplify(world_map, dTolerance = 500)

# Mapa para PDF (ggplot)
if (knitr::is_latex_output()) {
  mapa_felicidad_gg <- ggplot(world_map_s) +
    geom_sf(aes(fill = happiness_score)) +
    scale_fill_viridis_c(name = "Happiness Score") +
    theme_void() +
    labs(title = "Mapa de Felicidad por País (Sin democracia, 2020)")

  ggsave("mapa_felicidad_2020.png", mapa_felicidad_gg, width = 10, height = 6, dpi = 300)
  cat("![](mapa_felicidad_2020.png)")
} else {
  # Mapa interactivo para HTML
  fig <- plot_geo(world_map_s) %>%
    add_trace(
      z = ~happiness_score,
    ...
    layout(
      title = "Mapa de Felicidad por País (Sin democracia, 2020)",
      geo = list(
        showframe = FALSE,
...
p <- ggplot(df_corrupcion, aes(x = regional_indicator, y = perceptions_of_corruption, fill = regional_indicator)) +
  geom_violin(alpha = 0.6, trim = FALSE) +
  stat_summary(fun = median, geom = "point", color = "black", shape = 18, size = 2) +
  theme_minimal() +
  facet_wrap(~ year, ncol = 1) +
...
vars_interes <- c("happiness_score", "gdp_per_capita", "social_support", 
                  "healthy_life_expectancy", "freedom_to_make_life_choices", 
                  "generosity", "perceptions_of_corruption")
