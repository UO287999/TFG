...
# Reestructuramos a formato largo para ggplot
df_box_long <- df_box %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "valor")

# Gráfico unificado de boxplots
ggplot(df_box_long, aes(x = variable, y = valor, fill = variable)) +
...
variables_politicas <- c("regime_category", "is_monarchy", "is_democracy", 
                         "is_presidential", "is_colony", "is_communist", 
                         "has_full_suffrage", "electoral_category", 
                         "has_free_and_fair_election", "has_alternation")
...
paises_con_cambios <- c("Angola", "Bolivia", "Burkina Faso", "Burundi", "China", "Comoros",
                        "Congo (Kinshasa)", "Egypt", "Honduras", "Hungary", "Iraq", "Kenya",
                        "Laos", "Liberia", "Madagascar", "Myanmar", "Nicaragua", "Turkey",
                        "Venezuela", "Yemen", "Zimbabwe")

# Variables categóricas que han cambiado
variables_categoricas <- c("regime_category", "is_democracy", "is_presidential",
                           "electoral_category", "has_free_and_fair_election", "has_alternation")

# Transformar las variables a carácter para evitar conflictos en pivot_longer
df_cambios <- df_completo %>%
  filter(country %in% paises_con_cambios) %>%
  mutate(across(all_of(variables_categoricas), as.character)) %>%
  select(country, year, all_of(variables_categoricas)) %>%
  pivot_longer(cols = -c(country, year), names_to = "variable", values_to = "valor")

# Gráfico
ggplot(df_cambios, aes(x = year, y = valor, group = country, color = country)) +
  geom_line(linewidth = 0.8) +
  facet_wrap(~ variable, scales = "free_y") +
  theme_minimal() +
  labs(title = "Evolución de variables políticas en países con cambios",
       x = "Año", y = "Valor", color = "País") +
...
Si observamos la forma de gobierno presidencial, vemos que el único Estado que cambia es China: aunque siempre fue una dictadura de partido único, el dataset refleja un cambio en esta variable posiblemente por una reinterpretación metodológica, ya que a partir de 2019 es considerada presidencial.
...
```{r mapamundi, include=FALSE, warning=FALSE}
library(plotly)
library(dplyr)
library(sf)
library(rnaturalearth)
...
mapa_felicidad <- plot_ly(world_map, 
                          type = "choropleth",
                          locations = ~iso_a3,
                          z = ~happiness_score,
                          text = ~name,
...
if (knitr::is_latex_output()) {
  # Solo si quieres exportar a PDF y tienes phantomjs funcionando
  try({
    htmlwidgets::saveWidget(mapa_felicidad, "temp_map.html", selfcontained = TRUE)
    webshot2::webshot("temp_map.html", "mapa_felicidad_sin_democracia.png", vwidth = 1200, vheight = 800)
    cat("![](mapa_felicidad_sin_democracia.png)")
  }, silent = TRUE)
} else {
  mapa_felicidad
}
...
# Filtrar por los años solicitados
...
p <- ggplot(df_violin, aes(x = regional_indicator, y = happiness_score, fill = regional_indicator)) +
  geom_violin(alpha = 0.6, trim = FALSE) +  # trim=FALSE muestra mejor las colas
  stat_summary(fun = median, geom = "point", color = "black", shape = 18, size = 2) +  # Mediana
  theme_minimal() +
...
vars_evol <- df_sin_democracia_sin_na %>%
  group_by(year) %>%
  summarise(
...
# Gráfico
ggplot(vars_evol, aes(x = year, y = media, color = variable)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  facet_wrap(~variable, scales = "free_y") +
  theme_minimal() +
  labs(title = "Evolución anual del promedio de variables de interés",
       x = "Año", y = "Media") +
  theme(legend.position = "none")
```

El gráfico de la libertad para tomar decisiones destaca por su crecimiento constante, especialmente desde 2018. Este incremento podría reflejar mayores libertades percibidas por los ciudadanos en sus vidas personales, aunque también puede estar influido por cambios en algunos países concretos.

La evolución del PIB per cápita presenta un pico atípico en 2016, probablemente derivado de algún valor extremo en la base de datos. A partir de ahí, la evolución es más estable, aunque con ligeros repuntes a partir de 2021, posiblemente debido a la recuperación económica tras la crisis sanitaria.

La generosidad presenta un crecimiento sostenido desde 2016, con especial aceleración en los últimos tres años. Este repunte podría estar relacionado con respuestas solidarias durante la pandemia o con un cambio en los valores sociales.

La felicidad percibida muestra una tendencia creciente desde 2017 hasta alcanzar su punto máximo en 2022, seguido de un leve descenso en 2024. Esto sugiere una cierta recuperación post-pandemia, aunque sin volver a los niveles anteriores a 2017.

La esperanza de vida muestra una tendencia claramente ascendente, aunque con ciertos altibajos. El incremento sostenido sugiere mejoras en salud pública y condiciones de vida, aunque no es posible descartar la influencia de datos específicos de algunos países.

La percepción de corrupción presenta un patrón mucho más errático, con oscilaciones abruptas entre años. Esto refleja fuertes diferencias entre países o inestabilidades políticas concretas, y muestra que la percepción de corrupción no sigue una tendencia clara a nivel global.

El apoyo social se mantiene relativamente estable en el tiempo, con una caída puntual en 2018. A pesar de ligeras oscilaciones, no se detectan cambios estructurales fuertes, lo que podría indicar una percepción constante del apoyo social entre los ciudadanos.
