...
variables_binarias <- c("has_alternation", "has_free_and_fair_election", "is_democracy")
...
# Obtener mapa base y unir con datos de felicidad
...
# Usar iso_a3 como códigos de país
fig2 <- plot_geo(world_map) %>%
  add_trace(z = ~happiness_score, color = ~happiness_score, colorscale = "Viridis",
            text = ~name, locations = ~iso_a3, marker = list(line = l)) %>%
  colorbar(title = "Happiness Score") %>%
  layout(title = 'Mapa de Felicidad por País (rnaturalearth + plotly)', geo = g)
...
vars_interes <- c("gdp_per_capita", "social_support", 
                  "healthy_life_expectancy", "freedom_to_make_life_choices", 
...
# Unir la media con el dataframe original
df_plot <- df_sin_democracia_sin_na %>%
  filter(!is.na(happiness_score)) %>%
  left_join(media_anual, by = "year")

# Gráfico con facet por año y línea de la media global
ggplot(df_plot, aes(x = reorder(country, -happiness_score), y = happiness_score, fill = happiness_score)) +
  geom_col(show.legend = FALSE) +
  geom_hline(aes(yintercept = media_happiness), color = "red", linetype = "dashed", linewidth = 0.8) +
  facet_wrap(~ year, scales = "free_x") +
  scale_fill_viridis_c() +
  labs(
    title = "Happiness Score por País (con media global por año)",
